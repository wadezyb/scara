...
int main( int argc, char **argv )
{
  ...
  ros::Publisher pub=n.advertise<std_msgs::Int32MultiArray>("robotInfo",5);
		...
		if( serial.start != 0 )
		  {
			...
			if( serial.sum == serial.buf[BUF_LENGTH] )
			  {
			    pos.data[0]=((serial.buf[1]&0xff)<<24)+((serial.buf[2]&0xff)<<16)+((serial.buf[3]&0xff)<<8)+(serial.buf[4]&0xff);
			    pos.data[1]=((serial.buf[5]&0xff)<<24)+((serial.buf[6]&0xff)<<16)+((serial.buf[7]&0xff)<<8)+(serial.buf[8]&0xff);
			    pos.data[2]=((serial.buf[9]&0xff)<<24)+((serial.buf[10]&0xff)<<16)+((serial.buf[11]&0xff)<<8)+(serial.buf[12]&0xff);
			    pos.data[3]=((serial.buf[13]&0xff)<<24)+((serial.buf[14]&0xff)<<16)+((serial.buf[15]&0xff)<<8)+(serial.buf[16]&0xff);
			    pos.data[4]=((serial.buf[17]&0xff)<<24)+((serial.buf[18]&0xff)<<16)+((serial.buf[19]&0xff)<<8)+(serial.buf[20]&0xff);
